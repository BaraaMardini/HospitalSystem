...
const DEFAULTS = {
  title: "Rooms",
  description: "Manage rooms and their information.",
  addLabel: "Add Room",
};

const title = roomEntity.title || humanize(roomEntity.entity) || DEFAULTS.title;
const description = roomEntity.description || DEFAULTS.description;
const addLabel = roomEntity.addLabel || DEFAULTS.addLabel;

const columns = roomEntity.operations.getAll?.columns || roomEntity.operations.search?.columns || [];

const keyField = roomEntity.operations.update?.by || roomEntity.operations.delete?.by || roomEntity.idField || "id";

const hasSearch = Boolean(roomEntity.operations.search);
const hasAdd = Boolean(roomEntity.operations.add);
const hasUpdate = Boolean(roomEntity.operations.update);
const hasDelete = Boolean(roomEntity.operations.delete);
const hasActionsColumn = hasUpdate || hasDelete;

const addFields = roomEntity.operations.add?.fields || [];
const editFields = roomEntity.operations.update?.fields || [];

const searchFilters = roomEntity.operations.search?.filters || [];
const mainFilter = searchFilters.find((f) => f.type !== "number" && f.type !== "int" && !f.source);
const inlineFilters = searchFilters.filter((f) => f !== mainFilter);

const roomTypeSource =
  [...addFields, ...editFields, ...searchFilters].find((f) => f.source && f.source.entity === "RoomType")?.source ||
  null;

function badgeTone(value) {
  const v = String(value ?? "").toLowerCase();
  if (["active", "confirmed", "completed", "paid", "in stock", "ready", "admitted"].some((k) => v.includes(k)))
    return "bg-emerald-50 text-emerald-700 border-emerald-200";
  if (
    ["pending", "low stock", "processing", "under observation", "scheduled", "under review"].some((k) => v.includes(k))
  )
...
function validateField(field, value, dynamicOptions) {
  ...
  if (field.type === "number" && Number.isNaN(Number(value))) return "Enter a valid number.";
  if (field.type === "select") {
    const opts = field.source ? dynamicOptions || [] : field.options || [];
    if (opts.length > 0 && !opts.some((o) => String(o.value) === String(value))) {
...
function EntityFormModal({ open, title, fields, initialValues, submitLabel, loading, sourceOptionsMap, onCancel, onSubmit }) {
  ...
  const handleSubmit = (e) => {
    ...
    fields.forEach((field) => {
      const dynamicOptions = field.source ? sourceOptionsMap[field.name] : undefined;
      const err = validateField(field, values[field.name], dynamicOptions);
                ...
                {field.type === "textarea" ? (
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      {(field.source ? sourceOptionsMap[field.name] || [] : field.options || []).map((opt) => (
...
export default function RoomPage() {
  const { getAllState, fetchAll, searchState, search, addState, add, updateState, update, deleteState, remove } =
    useRoomStore();
  ...
  const roomTypeOptions = roomTypeSource
    ? (roomTypeStore.getAllState.data || []).map((r) => ({
        value: r[roomTypeSource.valueField],
        label: r[roomTypeSource.displayField],
      }))
    : [];

  const sourceOptionsMap = { roomTypeID: roomTypeOptions };

  const anyFilterActive = Object.values(filters).some((v) => v !== undefined && v !== null && String(v).trim() !== "");

  const rows = anyFilterActive ? searchState.data || [] : getAllState.data || [];
  const loading = anyFilterActive ? searchState.loading : getAllState.loading;
  ...
  const handleAdd = async (values) => {
    await add(values);
    const result = useRoomStore.getState().addState;
    if (result.errorCode === 0) {
      showToast({ type: "success", title: "Room created", description: result.message || `${title} created successfully` });
      setAddOpen(false);
      refresh();
    } else {
  ...
  const handleUpdate = async (values) => {
    ...
    const result = useRoomStore.getState().updateState;
    if (result.errorCode === 0) {
  ...
  const handleDelete = async () => {
    ...
    const result = useRoomStore.getState().deleteState;
    if (result.errorCode === 0) {
  ...
  const activeQuery = anyFilterActive ? filters[mainFilter?.name] || Object.values(filters).find((v) => v) || "" : "";
    ...
    <div className="min-h-full w-full bg-slate-50 text-slate-900">
      <div className="w-full px-5 py-6 sm:px-7 lg:px-9 xl:px-10">
        ...
        <section className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <StatCard icon={DoorOpen} label="Total records" value={totalRows} description="Records currently visible" />
          <StatCard
            icon={Search}
            label="Search"
            value={anyFilterActive ? "Active" : "All"}
            description={anyFilterActive ? "Search filters are active" : "Showing all records"}
          />
          <StatCard icon={DoorOpen} label="Results" value={totalRows} description="Current result count" />
          <StatCard
            icon={Plus}
            label="Management"
                  ...
                  {f.type === "number" || f.type === "int" ? (
                    ...
                    <div className="relative">
                      ...
                      >
                        ...
                        {(f.source.entity === "RoomType" ? roomTypeOptions : []).map((opt) => (
                          <option key={opt.value} value={opt.value}>
                            {opt.label}
                          </option>
                        ))}
        ...
        <section className="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white">
          <div className="flex flex-col gap-3 border-b border-slate-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between lg:px-6">
            <div>
              <h2 className="text-sm font-semibold text-slate-900">Room directory</h2>
              <p className="mt-1 text-xs text-slate-400">
                {anyFilterActive ? "Showing filtered results" : "Complete room directory"}
              </p>
            </div>
          ...
          {!loading && rows.length === 0 ? (
            <div className="flex min-h-[420px] flex-col items-center justify-center px-6 text-center">
              ...
              <h3 className="text-lg font-semibold text-slate-900">
                {anyFilterActive ? "No matching records" : `No ${title.toLowerCase()} yet`}
              </h3>
              <p className="mt-2 max-w-md text-sm leading-6 text-slate-500">
                {anyFilterActive
                  ? `Nothing matches "${activeQuery}". Try changing the filters.`
                  : "There are currently no records available in the system."}
              </p>
              {hasAdd && !anyFilterActive && (
                <button
                  type="button"
                  onClick={() => setAddOpen(true)}
                        ...
                        {columns.map((col, j) => (
                          <td key={col.field} className="px-6 py-5 align-middle">
                            {col.badge ? (
                              ...
                              <span className="font-medium text-slate-800">{String(row[col.field] ?? "—")}</span>
                            ) : (
                              <span className="text-sm text-slate-500">{String(row[col.field] ?? "—")}</span>
                            )}
                          </td>
                        ))}
                        {hasActionsColumn && (
      ...
      <ConfirmDialog open={Boolean(deleteRow)} loading={deleteState.loading} onCancel={() => setDeleteRow(null)} onConfirm={handleDelete} />
