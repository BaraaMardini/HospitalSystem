...
import useInsuranceSubscriptionsStore from "../stores/InsuranceSubscriptionsStore";
...
const config = insuranceSubscriptionsEntity;
const operations = config.operations || {};

const DEFAULTS = {
  title: "Insurance Subscriptions",
  description: "Manage insurance subscription records.",
...
const hasUpdate = Boolean(operations.update); // not declared on this entity — no Edit icon/modal
...
// PersonName / CompanyName are relational selects (source).
// CoveragePercentage is numeric -> compact inline number input.
// EndDate is a plain DateTime filter -> its own inline datetime-local input.
...
export default function InsuranceSubscriptionsPage() {
  const {
    ...
    deleteState,
    remove,
  } = useInsuranceSubscriptionsStore();
  ...
  const companyNameSource = searchFilters.find((f) => f.name === "CompanyName")?.source;

  const personNameOptions = useSourceOptions(personNameSource);
  const companyNameOptions = useSourceOptions(companyNameSource);
  ...
  // --- options for the Add form selects ---
  const insuranceIdSource = operations.add?.fields.find(
    (f) => f.name === "insuranceID"
  )?.source;
  const personIdSource = operations.add?.fields.find((f) => f.name === "personID")?.source;

  const insuranceIdOptions = useSourceOptions(insuranceIdSource);
  const personIdOptions = useSourceOptions(personIdSource);

  const formSourceOptions = {
    insuranceID: insuranceIdOptions,
  ...
  const handleAdd = async (values) => {
    await add(values);
    const result = useInsuranceSubscriptionsStore.getState().addState;
    if (result.errorCode === 0) {
      ...
      showToast({ type: "error", title: "Unable to create subscription", description: result.message });
    }
  };

  const handleDelete = async () => {
    await remove(deleteRow[keyField]);
    const result = useInsuranceSubscriptionsStore.getState().deleteState;
    if (result.errorCode === 0) {
      showToast({
        type: "success",
      ...
      showToast({ type: "error", title: "Unable to delete subscription", description: result.message });
    }
    setDeleteRow(null);
    refresh();
  };
      ...
      <div className="w-full px-5 py-6 sm:px-7 lg:px-9 xl:px-10">
        ...
        <section className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <StatCard
            ...
            description={hasAdd ? "Create records" : "Read-only access"}
          />
        </section>
        ...
        <section className="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white">
          <div className="flex flex-col gap-3 border-b border-slate-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between lg:px-6">
            <div>
              <h2 className="text-sm font-semibold text-slate-900">Subscription records</h2>
              <p className="mt-1 text-xs text-slate-400">
                {hasActiveFilter ? "Showing filtered results" : "Complete subscription list"}
              </p>
            </div>
