...
const config = PeopleEntity;

const DEFAULTS = {
  title: "People",
  description: "Manage people records and information.",
  addLabel: "Add Person",
};

const columns = (
  config.operations.getAll?.columns ||
  config.operations.search?.columns ||
  []
).map((c) => ({
  key: c.field,
  label: c.header,
...
const filterFields = config.operations.search?.filters || [];
...
export default function PeoplePage() {
  ...
  async function handleAddSubmit(values) {
    ...
    const { errorCode, message } =
      usePeopleStore.getState().addState;
    ...
    if (errorCode === 0) {
      showToast({
        ...
        description:
          message ||
          `${config.title || DEFAULTS.title} created successfully.`,
      });
  ...
  async function handleEditSubmit(values) {
    ...
    const { errorCode, message } =
      usePeopleStore.getState().updateState;
  ...
  async function handleDeleteConfirm() {
    ...
    const { errorCode, message } =
      usePeopleStore.getState().deleteState;
  ...
  const editFields =
    config.operations.update?.fields || [];

  const addFields =
    config.operations.add?.fields || [];
    ...
    <div className="min-h-full w-full bg-slate-50 text-slate-900">
      ...
      {hasUpdate && (
        <EntityFormModal
          ...
          title={`Edit ${(config.title || DEFAULTS.title).replace(
            /s$/,
            ""
          )}`}
          fields={editFields}
