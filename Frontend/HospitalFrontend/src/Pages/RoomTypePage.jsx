...
const title = roomTypeEntity.title || humanize(roomTypeEntity.entity);
const description = roomTypeEntity.description || "";
const addLabel = roomTypeEntity.addLabel || `Add ${title}`;

const columns =
  roomTypeEntity.operations.getAll?.columns ||
  roomTypeEntity.operations.search?.columns ||
  [];

const keyField =
  roomTypeEntity.operations.update?.by ||
  roomTypeEntity.operations.delete?.by ||
  roomTypeEntity.idField ||
  "id";

const hasSearch = Boolean(roomTypeEntity.operations.search);
const hasAdd = Boolean(roomTypeEntity.operations.add);
const hasUpdate = Boolean(roomTypeEntity.operations.update);
const hasDelete = Boolean(roomTypeEntity.operations.delete);
const hasActionsColumn = hasUpdate || hasDelete;

const addFields = roomTypeEntity.operations.add?.fields || [];
const editFields = roomTypeEntity.operations.update?.fields || [];
...
// ============================================================
// TOAST (same hook/behavior, restyled view)
// ============================================================
...
function EntityFormModal({ title, fields, initialValues, submitLabel, loading, onCancel, onSubmit }) {
        ...
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
          <div>
            <p className="mb-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-emerald-600">
              {humanize(roomTypeEntity.entity)} management
            </p>
              ...
              <div key={field.name} className={field.type === "textarea" ? "md:col-span-2" : ""}>
                ...
                {field.type === "select" ? (
                  <div className="relative">
                    <select
                      className="w-full appearance-none rounded-xl border border-slate-200 bg-white px-4 py-3 pr-10 text-sm text-slate-900 outline-none transition focus:border-emerald-400 focus:ring-4 focus:ring-emerald-500/10"
                  ...
                  <textarea
                    rows={4}
                    className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-emerald-400 focus:ring-4 focus:ring-emerald-500/10"
...
export default function RoomTypePage() {
  ...
  const handleAdd = async (values) => {
    await add(values);
    const result = useRoomTypeStore.getState().addState;
  ...
  const handleUpdate = async (values) => {
    ...
    const result = useRoomTypeStore.getState().updateState;
    if (result.errorCode === 0) {
      showToast({
        type: "success",
        title: "Success",
  ...
  const handleDelete = async () => {
    ...
    const result = useRoomTypeStore.getState().deleteState;
    if (result.errorCode === 0) {
      showToast({
        type: "success",
        title: "Success",
