...
import useInvoicePaymentsStore from "../stores/InvoicePaymentsStore";
...
const config = invoicePaymentsEntity;
const operations = config.operations || {};

const DEFAULTS = {
  title: "Invoice Payments",
  description: "Manage payments recorded against invoices.",
...
// InvoiceID declares `type: "int"` AND a `source` — `source` presence
// always wins over the declared type, so it renders as an inline select
// (of invoice IDs), not a number input. PersonName also has a `source`.
...
// ============================================================
// SOURCE OPTIONS — calls the raw API layer directly (not the shared
// entity store) so this dropdown's data can never be clobbered by
// another page/field also reading the same referenced entity through
// its global store.
// ============================================================
...
function useSourceOptions(source, filters) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      let result;
      if (op === "search" && api.search) {
        result = await api.search(filters || {});
      } else if (api.getAll) {
...
export default function InvoicePaymentsPage() {
  ...
  const invoiceIdFilterSource = searchFilters.find((f) => f.name === "InvoiceID")?.source;
  const personNameFilterSource = searchFilters.find((f) => f.name === "PersonName")?.source;

  const invoiceIdFilterOptions = useSourceOptions(invoiceIdFilterSource);
  const personNameOptions = useSourceOptions(personNameFilterSource);

  const filterOptionsByField = {
    InvoiceID: invoiceIdFilterOptions,
    PersonName: personNameOptions,
  };

  // --- options for the Add/Edit form selects ---
  const invoiceIdFieldSource = operations.add?.fields.find(
    (f) => f.name === "invoiceID"
  )?.source;

  const invoiceIdOptions = useSourceOptions(invoiceIdFieldSource);
  ...
  const handleAdd = async (values) => {
    await add(values);
    const result = useInvoicePaymentsStore.getState().addState;
    if (result.errorCode === 0) {
      ...
      showToast({ type: "error", title: "Unable to create payment", description: result.message });
    }
  };

  const handleUpdate = async (values) => {
    await update(editRow[keyField], values);
    const result = useInvoicePaymentsStore.getState().updateState;
    if (result.errorCode === 0) {
      showToast({
        type: "success",
      ...
      showToast({ type: "error", title: "Unable to update payment", description: result.message });
    }
  };

  const handleDelete = async () => {
    await remove(deleteRow[keyField]);
    const result = useInvoicePaymentsStore.getState().deleteState;
    if (result.errorCode === 0) {
      showToast({
        type: "success",
      ...
      showToast({ type: "error", title: "Unable to delete payment", description: result.message });
    }
    setDeleteRow(null);
    refresh();
  };
      ...
      <div className="w-full px-5 py-6 sm:px-7 lg:px-9 xl:px-10">
        ...
        <section className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <StatCard
            ...
            description={hasActiveFilter ? "Search filters are active" : "Showing all records"}
          />
          <StatCard
            icon={CreditCard}
            label="Results"
        ...
        <section className="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white">
          <div className="flex flex-col gap-3 border-b border-slate-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between lg:px-6">
            <div>
              <h2 className="text-sm font-semibold text-slate-900">Payment records</h2>
              <p className="mt-1 text-xs text-slate-400">
                {hasActiveFilter ? "Showing filtered results" : "Complete payment list"}
              </p>
            </div>
          ...
          {!loading && rows.length === 0 ? (
            <div className="flex min-h-[420px] flex-col items-center justify-center px-6 text-center">
              ...
              <h3 className="text-lg font-semibold text-slate-900">
                {hasActiveFilter ? "No matching payments" : `No ${title.toLowerCase()} yet`}
              </h3>
          ...
          {rows.length > 0 && !loading && (
            <div className="flex flex-col gap-3 border-t border-slate-100 px-5 py-4 text-xs text-slate-400 sm:flex-row sm:items-center sm:justify-between lg:px-6">
              <span>
                Showing <span className="font-medium text-slate-600">{rows.length}</span> payment records
              </span>
              <span>
