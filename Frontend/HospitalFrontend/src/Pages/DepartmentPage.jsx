...
const config = DepartmentEntity;

const DEFAULTS = {
  title: "Departments",
  description: "Manage department records and information.",
  addLabel: "Add Department",
};

const columns = (config.operations?.getAll?.columns || []).map((c) => ({
  key: c.field,
  label: c.header,
...
function badgeTone(value) {
  ...
  if (
    ["cancelled", "overdue", "out of stock", "on leave"].some((s) =>
      v.includes(s)
    )
  ) {
...
export default function DepartmentPage() {
  ...
  async function handleAddSubmit(values) {
    ...
    const { errorCode, message } = useDepartmentStore.getState().addState;
    ...
    if (errorCode === 0) {
      showToast({
        ...
        description:
          message || `${config.title || DEFAULTS.title} created successfully.`,
      });
  ...
  async function handleEditSubmit(values) {
    ...
    const { errorCode, message } = useDepartmentStore.getState().updateState;

    if (errorCode === 0) {
      showToast({
        type: "success",
        title: "Record updated",
        description: message || "The record was updated successfully.",
      });
  ...
  async function handleDeleteConfirm() {
    ...
    const { errorCode, message } = useDepartmentStore.getState().deleteState;

    if (errorCode === 0) {
      showToast({
        type: "success",
        title: "Record deleted",
        description: message || "The record was deleted successfully.",
      });
  ...
  const editFields = config.operations?.update?.fields || [];
  const addFields = config.operations?.add?.fields || [];
  ...
  return (
    <div className="min-h-full w-full bg-slate-50 text-slate-900">
      <div className="w-full px-5 py-6 sm:px-7 lg:px-9 xl:px-10">
            ...
            <div className="flex items-center gap-3">
              ...
              {hasAdd && (
                ...
                >
                  ...
                  {config.addLabel || DEFAULTS.addLabel}
                </button>
              )}
            </div>
          </div>
        ...
        <section className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <StatCard
            ...
            label="Filters"
            value="—"
            description="Not available for this entity"
          />
        ...
        {/* ====================================================
            TOOLBAR (no search operation on this entity — filters
            affordance kept for visual/behavioral parity)
        ==================================================== */}
          ...
          {!loading && rows.length === 0 ? (
            <div className="flex min-h-[420px] flex-col items-center justify-center px-6 text-center">
              ...
              <h3 className="text-lg font-semibold text-slate-900">
                {`No ${(config.title || DEFAULTS.title).toLowerCase()} yet`}
              </h3>
      ...
      {hasAdd && (
        <EntityFormModal
          open={addOpen}
          title={config.addLabel || DEFAULTS.addLabel}
          fields={addFields}
          initialValues={{}}
      ...
      {hasUpdate && (
        <EntityFormModal
          ...
          title={`Edit ${(config.title || DEFAULTS.title).replace(/s$/, "")}`}
          fields={editFields}
      ...
      {hasDelete && (
        <ConfirmDialog
          ...
          description="This action permanently removes the selected record from the system and cannot be undone."
