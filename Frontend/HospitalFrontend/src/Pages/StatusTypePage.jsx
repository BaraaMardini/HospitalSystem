...
export default function StatusTypePage() {
  ...
  const handleAdd = async (values) => {
    await add(values);
    const result = useStatusTypeStore.getState().addState;
  ...
  const handleUpdate = async (values) => {
    ...
    const result = useStatusTypeStore.getState().updateState;
    if (result.errorCode === 0) {
      showToast({
        type: "success",
        title: "Success",
  ...
  const handleDelete = async () => {
    ...
    const result = useStatusTypeStore.getState().deleteState;
    if (result.errorCode === 0) {
      showToast({
        type: "success",
        title: "Success",
