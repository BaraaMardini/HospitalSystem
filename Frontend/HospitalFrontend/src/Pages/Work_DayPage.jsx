...
import useWork_DayStore from "../stores/Work_DayStore";
...
const title = work_DayEntity.title || "Work Days";
const description = work_DayEntity.description || "";
const addLabel = work_DayEntity.addLabel || `Add ${title}`;

const operations = work_DayEntity.operations || {};
...
const keyField = operations.update?.by || operations.delete?.by || work_DayEntity.idField || "id";
...
function EntityFormModal({ fields, initialValues, submitting, onCancel, onSubmit }) {
        ...
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
          <div>
            ...
            <h3 className="text-xl font-semibold tracking-tight text-slate-900">
              {initialValues && initialValues[keyField] ? `Edit ${title}` : addLabel}
            </h3>
          </div>
          <button
            type="button"
...
export default function Work_DayPage() {
  const store = useWork_DayStore();
  const { getAllState, fetchAll, add, update, remove } = store;
  ...
  const handleAddSubmit = async (values) => {
    ...
    await add?.(values);
    setSubmitting(false);
    const result = useWork_DayStore.getState().addState;
    if (result.errorCode === 0) {
      showToast({ type: "success", title: "Success", description: result.message || `${title} created successfully` });
      setShowAdd(false);
      refresh();
    } else {
  ...
  const handleEditSubmit = async (values) => {
    ...
    setSubmitting(false);
    const result = useWork_DayStore.getState().updateState;
    if (result.errorCode === 0) {
      showToast({ type: "success", title: "Success", description: result.message || `${title} updated successfully` });
      setEditRow(null);
      refresh();
    } else {
      showToast({ type: "error", title: "Error", description: result.message });
  ...
  const handleDeleteConfirm = async () => {
    ...
    const result = useWork_DayStore.getState().deleteState;
    if (result.errorCode === 0) {
      showToast({ type: "success", title: "Success", description: result.message || `${title} deleted successfully` });
    } else {
      showToast({ type: "error", title: "Error", description: result.message });
    }
    setDeleteRow(null);
  ...
  return (
    <div className="min-h-full w-full bg-slate-50 text-slate-900">
      ...
      {deleteRow && <ConfirmDialog onCancel={() => setDeleteRow(null)} onConfirm={handleDeleteConfirm} />}
