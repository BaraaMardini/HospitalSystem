...
const config = insuranceEntity;
const operations = config.operations || {};

const DEFAULTS = {
  title: "Insurances",
  description: "Manage insurance companies and coverage plans.",
...
function coverageBadgeTone(value) {
  ...
  if (!Number.isNaN(number)) {
    if (number >= 80) return "bg-emerald-50 text-emerald-700 border-emerald-200";
    if (number >= 50) return "bg-amber-50 text-amber-700 border-amber-200";
    if (number > 0) return "bg-red-50 text-red-700 border-red-200";
  }
...
function validateFields(fields, values) {
  ...
  for (const field of fields) {
    ...
    if (
      field.name === "coveragePercentage" &&
      !Number.isNaN(Number(value)) &&
      (Number(value) < 0 || Number(value) > 100)
    ) {
      errors[field.name] = "Coverage must be between 0 and 100.";
    }
...
export default function InsurancePage() {
  ...
  const totalInsurances = rows.length;

  const companyCount = new Set(
    rows.map((row) => row.companyName).filter(Boolean)
  ).size;

  const avgCoverage = (() => {
    const values = rows
      .map((row) => Number(row.coveragePercentage))
  ...
  const handleAdd = async (values) => {
    ...
    const result = useInsuranceStore.getState().addState;
    ...
    if (result.errorCode === 0) {
      ...
      showToast({
        type: "error",
        title: "Unable to create insurance",
  ...
  const handleUpdate = async (values) => {
    ...
    const result = useInsuranceStore.getState().updateState;
    ...
    if (result.errorCode === 0) {
      ...
      showToast({
        type: "error",
        title: "Unable to update insurance",
        description: result.message,
      });
    }
  };
  ...
  const handleDelete = async () => {
    ...
    const result = useInsuranceStore.getState().deleteState;
    ...
    if (result.errorCode === 0) {
      ...
      showToast({
        type: "error",
        title: "Unable to delete insurance",
        description: result.message,
      });
    }
      ...
      <div className="w-full px-5 py-6 sm:px-7 lg:px-9 xl:px-10">
        ...
        <section className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <StatCard
            ...
            label="Avg. coverage"
            value={avgCoverage}
            description="Across visible plans"
          />
            ...
            <div className="flex min-h-[420px] flex-col items-center justify-center px-6 text-center">
              ...
              <p className="mt-2 max-w-md text-sm leading-6 text-slate-500">
                {hasActiveFilter
                  ...
                  : "Once insurance plans are created, they will appear here."}
              </p>
                    ...
                    {rows.map((row, index) => (
                      ...
                      >
                        {columns.map((col, j) => {
                          ...
                          if (col.field === "coveragePercentage") {
                            return (
                              <td key={col.field} className="px-6 py-5">
                                <span
                                  className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] font-semibold ${coverageBadgeTone(
                                    value
                                  )}`}
                                >
                          ...
                          if (col.field === "defaultDurationMonths") {
                            return (
                              <td key={col.field} className="px-6 py-5">
