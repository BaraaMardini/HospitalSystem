...
const config = appointmentEntity;
const operations = config.operations || {};

const DEFAULTS = {
  title: "Appointments",
  description: "Manage, monitor and coordinate patient appointments.",
...
function isDateTimeType(type) {
  return (
    (typeof type === "string" &&
      type.toLowerCase().replace("-", "") === "datetime") ||
    type === "datetime-local"
  );
}
...
// ============================================================
// SOURCE OPTIONS (dropdown data coming from other entities)
// ============================================================
...
// ============================================================
// BADGE (status-like columns)
// ============================================================

function isStatusLikeColumn(field) {
...
function badgeTone(value) {
  ...
  if (
    ["completed", "confirmed", "active", "paid", "done"].some((s) =>
      v.includes(s)
    )
  ) {
  ...
  if (
    ["cancelled", "canceled", "failed", "inactive", "rejected"].some((s) =>
      v.includes(s)
    )
  ) {
  ...
  if (
    ["pending", "scheduled", "waiting", "in progress"].some((s) =>
      v.includes(s)
    )
  ) {
      ...
      <div className="w-full max-w-md overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl">
        <div className="p-6">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-500">
            This record will be permanently removed from the system and
            cannot be undone.
          </p>
        </div>
...
export default function AppointmentPage() {
  ...
  const doctorNameSource = searchFilters.find(
    (f) => f.name === "DoctorName"
  )?.source;

  const patientNameSource = searchFilters.find(
    (f) => f.name === "PatientName"
  )?.source;

  const roomNumberSource = searchFilters.find(
    (f) => f.name === "RoomNumber"
  )?.source;

  const doctorNameOptions = useSourceOptions(doctorNameSource);
  const patientNameOptions = useSourceOptions(patientNameSource);
  const roomNumberOptions = useSourceOptions(roomNumberSource);

  const filterOptionsByField = {
    DoctorName: doctorNameOptions,
    PatientName: patientNameOptions,
    RoomNumber: roomNumberOptions,
  };
  ...
  const patientIdSource = operations.add?.fields?.find(
    (f) => f.name === "patientID"
  )?.source;

  const doctorIdSource = operations.add?.fields?.find(
    (f) => f.name === "doctorID"
  )?.source;
  ...
  const roomIdSource = operations.add?.fields?.find(
    (f) => f.name === "roomID"
  )?.source;

  const patientIdOptions = useSourceOptions(patientIdSource);
  const doctorIdOptions = useSourceOptions(doctorIdSource);

  const statusIdOptions = useSourceOptions(statusIdSource, {
    StatusTypeCode: "Appointment",
  });

  const roomIdOptions = useSourceOptions(roomIdSource);

  const formSourceOptions = {
    patientID: patientIdOptions,
    doctorID: doctorIdOptions,
    statusID: statusIdOptions,
    roomID: roomIdOptions,
  };
  ...
  const pendingAppointments = rows.filter((row) => {
    const value = String(row.StatusName ?? row.statusName ?? "").toLowerCase();
    return (
  ...
  const handleAdd = async (values) => {
    ...
    const result = useAppointmentStore.getState().addState;
  ...
  const handleUpdate = async (values) => {
    ...
    const result = useAppointmentStore.getState().updateState;
  ...
  const handleDelete = async () => {
    ...
    const result = useAppointmentStore.getState().deleteState;
      ...
      <div className="w-full px-5 py-6 sm:px-7 lg:px-9 xl:px-10">
        ...
        <section className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <StatCard
            ...
            description="Confirmed or finished"
          />

          <StatCard
            icon={Users}
            label="Current view"
            value={hasActiveFilter ? "Filtered" : "All"}
                    ...
                    {field.source ? (
                      <>
                        ...
                        >
                          ...
                          {(filterOptionsByField[field.name] || []).map(
                            (opt, idx) => (
                              <option key={opt.value ?? idx} value={opt.value ?? ""}>
                                {opt.label ?? opt.value ?? ""}
                              </option>
            ...
            <div className="flex min-h-[420px] flex-col items-center justify-center px-6 text-center">
              ...
              <p className="mt-2 max-w-md text-sm leading-6 text-slate-500">
                {hasActiveFilter
                  ...
                  : "Once appointments are created, they will appear here."}
              </p>
              ...
              {hasActiveFilter && (
                <button
                  type="button"
                  onClick={clearFilters}
                  className="mt-6 inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-5 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-slate-50"
                >
                  Clear filters
                </button>
              )}
