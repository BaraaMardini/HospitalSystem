...
// AppointmentID is numeric -> compact inline number input.
// PersonName / StatusName both declare a `source`, so per Section 3.3 they
// render as inline selects regardless of their declared `type: "string"` —
// `source` presence always wins over a plain string type.
...
function isDateTimeType(type) {
  return (
    (typeof type === "string" && type.toLowerCase().replace("-", "") === "datetime") ||
    type === "datetime-local"
  );
}
...
function nativeInputType(type) {
  ...
  if (isDateTimeType(type)) return "datetime-local";
  return "text";
}
...
function useSourceOptions(source, filters) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const declaredOp = (source.operation || "getAll").toLowerCase();
      const op = filters ? "search" : declaredOp;
...
function badgeTone(value) {
  ...
  if (
    ["active", "confirmed", "completed", "paid", "in stock", "ready", "admitted"].some((s) =>
      v.includes(s)
    )
  ) {
  ...
  if (["cancelled", "overdue", "out of stock", "on leave"].some((s) => v.includes(s))) {
      ...
      <div className="w-full max-w-md overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl">
        <div className="p-6">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-500">
            This action permanently removes the selected record from the system and cannot be
            undone.
          </p>
        </div>
...
function EntityFormModal({
  ...
  function handleSubmit(e) {
    e.preventDefault();
    const nextErrors = validateFields(fields, values, sourceOptionsByField);
...
export default function InvoicesPage() {
  ...
  // --- options for the toolbar's relational (source) filters ---
  const personNameSource = searchFilters.find((f) => f.name === "PersonName")?.source;
  const statusNameSource = searchFilters.find((f) => f.name === "StatusName")?.source;

  const personNameOptions = useSourceOptions(personNameSource);
  const statusNameOptions = useSourceOptions(statusNameSource, {
    StatusTypeCode: "INVOICE",
  });
  ...
  const appointmentIdSource = operations.add?.fields.find(
    (f) => f.name === "appointmentID",
  )?.source;
  const statusIdSource = operations.add?.fields.find((f) => f.name === "statusID")?.source;

  const appointmentIdOptions = useSourceOptions(appointmentIdSource);
  const statusIdOptions = useSourceOptions(statusIdSource, {
    StatusTypeCode: "INVOICE",
  });

  const formSourceOptions = {
    appointmentID: appointmentIdOptions,
  ...
  const activeQuery = Object.values(filters).some(
    (v) => v !== "" && v !== undefined && v !== null,
  );
  const rows = activeQuery ? searchState?.data || [] : getAllState?.data || [];
  const loading = activeQuery ? searchState?.loading : getAllState?.loading;
  ...
  const handleAdd = async (values) => {
    await add(values);
    const result = useInvoicesStore.getState().addState;
  ...
  const handleUpdate = async (values) => {
    ...
    const result = useInvoicesStore.getState().updateState;
    if (result.errorCode === 0) {
      showToast({
        type: "success",
        title: "Record updated",
        description: result.message || "The record was updated successfully.",
      });
      setEditRow(null);
      refresh();
    } else {
  ...
  const handleDelete = async () => {
    ...
    const result = useInvoicesStore.getState().deleteState;
    if (result.errorCode === 0) {
      showToast({
        type: "success",
        title: "Record deleted",
        description: result.message || "The record was deleted successfully.",
      });
    } else {
    ...
    <div className="min-h-full w-full bg-slate-50 text-slate-900">
      <div className="w-full px-5 py-6 sm:px-7 lg:px-9 xl:px-10">
        ...
        <section className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <StatCard icon={Receipt} label="Total records" value={totalRows} description="Records currently visible" />
          <StatCard
            icon={Search}
            label="Search"
            value={activeQuery ? "Active" : "All"}
            description={activeQuery ? "Search filters are active" : "Showing all records"}
          />
          <StatCard icon={Receipt} label="Results" value={totalRows} description="Current result count" />
          <StatCard
            icon={Plus}
            label="Management"
                  ...
                  {filter.source ? (
                    <div className="relative">
                      ...
                      >
                        ...
                        {(filterOptionsByField[filter.name] || []).map((opt, idx) => (
                          <option key={opt.value ?? idx} value={opt.value ?? ""}>
                            {opt.label ?? opt.value ?? ""}
                          </option>
                        ))}
        ...
        <section className="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white">
          <div className="flex flex-col gap-3 border-b border-slate-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between lg:px-6">
            <div>
              <h2 className="text-sm font-semibold text-slate-900">Invoices directory</h2>
              <p className="mt-1 text-xs text-slate-400">
                {activeQuery ? "Showing filtered results" : "Complete invoices directory"}
              </p>
            </div>
          ...
          {!loading && rows.length === 0 ? (
            <div className="flex min-h-[420px] flex-col items-center justify-center px-6 text-center">
              ...
              <h3 className="text-lg font-semibold text-slate-900">
                {activeQuery ? "No matching records" : `No ${title.toLowerCase()} yet`}
              </h3>
