...
import useWork_ScheduleStore from "../stores/Work_ScheduleStore";
...
const config = work_ScheduleEntity;
...
const sourceEntities = {
  Doctor: doctorEntity,
  Work_Day: work_DayEntity,
};

// UX request: the Add form's `dayID` select becomes a multi-select checkbox
// group — pick one doctor, tick every day they work, and Save creates one
// record per selected day in a single action (see handleAdd below). Edit
// stays a normal single select since a row is always exactly one record.
const ADD_MULTI_SELECT_FIELDS = ["dayID"];
...
function validateFields(fields, values, sourceOptionsByField, multiSelectFields = []) {
  const errors = {};
  for (const field of fields) {
    if (multiSelectFields.includes(field.name)) {
      const selected = values[field.name] || [];
      if (field.required !== false && selected.length === 0) {
        errors[field.name] = "Select at least one day.";
      }
      continue;
    }
...
function EntityFormModal({
  ...
  const [values, setValues] = useState(() => {
    ...
    multiSelectFields.forEach((name) => {
      if (!Array.isArray(init[name])) init[name] = init[name] ? [init[name]] : [];
    });
    return init;
  });
  ...
  const toggleMultiValue = (name, optionValue) => {
    setValues((prev) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const validationErrors = validateFields(fields, values, sourceOptionsByField, multiSelectFields);
                    ...
                    <div className="flex flex-wrap gap-2">
                      {options.map((opt, idx) => {
                        ...
                        return (
                          <label
                            key={opt.value ?? idx}
                            className={`flex cursor-pointer items-center gap-1.5 rounded-xl border px-3 py-2 text-sm transition ${
                              checked
...
export default function Work_SchedulePage() {
  const {
    getAllState,
    fetchAll,
    searchState,
  ...
  const doctorIdSource = operations.add?.fields.find((f) => f.name === "doctorID")?.source;
  const dayIdSource = operations.add?.fields.find((f) => f.name === "dayID")?.source;

  const doctorIdOptions = useSourceOptions(doctorIdSource);
  const dayIdOptions = useSourceOptions(dayIdSource);
  ...
  // One doctor + N selected days -> N sequential `add` calls, one Save
  // click, one combined toast.
  const handleAdd = async (values) => {
    const dayIds = values.dayID || [];
    const results = [];
    for (const dayID of dayIds) {
      await add({ doctorID: values.doctorID, dayID });
      results.push(useWork_ScheduleStore.getState().addState);
    }
    ...
    if (failures.length === 0) {
      showToast({
        ...
        description:
          results[0]?.message ||
          (results.length === 1 ? `${title} created successfully` : `${results.length} schedules created successfully`),
      });
    } else if (successes.length > 0) {
      showToast({
        type: "error",
        title: "Partially completed",
        description: `${successes.length} created, ${failures.length} failed: ${failures
          .map((f) => f.message)
          .filter(Boolean)
          .join("; ")}`,
      });
    } else {
      showToast({ type: "error", title: "Error", description: failures[0]?.message });
    }
  ...
  const handleUpdate = async (values) => {
    ...
    const result = useWork_ScheduleStore.getState().updateState;
  ...
  const handleDelete = async () => {
    ...
    const result = useWork_ScheduleStore.getState().deleteState;
    if (result.errorCode === 0) {
      showToast({
        type: "success",
        title: "Success",
    ...
    <div className="min-h-full w-full bg-slate-50 text-slate-900">
              ...
              {searchFilters.map((filter) => (
                <div key={filter.name}>
                  ...
                  <input
                    ...
                    type={filter.type === "number" || filter.type === "int" ? "number" : "text"}
      ...
      {addOpen && (
        <EntityFormModal
          ...
          multiSelectFields={ADD_MULTI_SELECT_FIELDS}
