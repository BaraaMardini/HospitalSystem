...
public static async Task<ApiResult<List<InsuranceViewDTO>>> SearchInsurance(
    string? CompanyName, int? CoveragePercentage, int? DefaultDurationMonths,
    CancellationToken cancellationToken = default)
{
    var result = await InsuranceData.SearchInsurance(
        CompanyName, CoveragePercentage, DefaultDurationMonths,
        cancellationToken);
