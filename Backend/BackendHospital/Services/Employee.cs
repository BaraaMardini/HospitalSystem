...
public static async Task<ApiResult<List<EmployeeViewDTO>>> SearchEmployee(
    string? Name, string? RoleName, string? DepartmentName, string? StatusName,
    CancellationToken cancellationToken = default)
{
    var result = await EmployeeData.SearchEmployee(
        Name, RoleName, DepartmentName, StatusName,
        cancellationToken);
