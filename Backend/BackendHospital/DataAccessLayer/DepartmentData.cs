    ...
    CancellationToken cancellationToken = default)
{
    ...
    public static async Task<ApiResult<DepartmentDTO>> UpdateDepartmentByIDAsync(
...
DepartmentDTO departmentDTO = new  DepartmentDTO();
    ...
    try
    {
        ...
        if (reader.Read())
        {
            departmentDTO.ID = reader.IsDBNull(reader.GetOrdinal("ID")) ? 0 : reader.GetInt32(reader.GetOrdinal("ID"));
            departmentDTO.Name = reader.IsDBNull(reader.GetOrdinal("Name")) ? null : reader.GetString(reader.GetOrdinal("Name"));
        }
        reader.Close();
