...
public static async Task<ApiResult<SpecializationDTO>> UpdateSpecializationByidAsync(
...
SpecializationDTO specializationDTO = new  SpecializationDTO();
    ...
    try
    {
        ...
        if (reader.Read())
        {
            specializationDTO.id = reader.IsDBNull(reader.GetOrdinal("id")) ? 0 : reader.GetInt32(reader.GetOrdinal("id"));
            specializationDTO.name = reader.IsDBNull(reader.GetOrdinal("name")) ? null : reader.GetString(reader.GetOrdinal("name"));
        }
        reader.Close();
        result.Data = specializationDTO;
        result.Message = outputMsg.Value?.ToString();
        result.ErrorType = ErrorTypeMapper.GetErrorType(Convert.ToInt32(outputErrorType.Value));
    }
    catch (Exception ex)
