...
public static async Task<ApiResult<Work_DayDTO>> UpdateWork_DayByIDAsync(
    Work_DayUpdateDTO work_day,
    CancellationToken cancellationToken = default)
{
Work_DayDTO work_dayDTO = new  Work_DayDTO();
    var result = new ApiResult<Work_DayDTO>();
    try
    {
        await using var connection = new SqlConnection(connectionString._connectionString);
        ...
        if (reader.Read())
        {
            work_dayDTO.ID = reader.IsDBNull(reader.GetOrdinal("ID")) ? 0 : reader.GetInt32(reader.GetOrdinal("ID"));
            work_dayDTO.DayName = reader.IsDBNull(reader.GetOrdinal("DayName")) ? null : reader.GetString(reader.GetOrdinal("DayName"));
        }
        reader.Close();
        result.Data = work_dayDTO;
        result.Message = outputMsg.Value?.ToString();
        result.ErrorType = ErrorTypeMapper.GetErrorType(Convert.ToInt32(outputErrorType.Value));
    }
    catch (Exception ex)
