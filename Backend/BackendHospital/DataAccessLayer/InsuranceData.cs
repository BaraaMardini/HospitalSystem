    ...
    CancellationToken cancellationToken = default)
{
    ...
    try
    {
        ...
        command.Parameters.Add("@Price", SqlDbType.Decimal).Value = insurance.Price;
...
public static async Task<ApiResult<InsuranceDTO>> UpdateInsuranceByIDAsync(
...
InsuranceDTO insuranceDTO = new  InsuranceDTO();
    ...
    try
    {
        ...
        command.Parameters.Add("@Price", SqlDbType.Decimal).Value = (object?)insurance.Price ?? DBNull.Value;
        var outputMsg = new SqlParameter("@Message", SqlDbType.NVarChar, 250) { Direction = ParameterDirection.Output };
        var outputErrorType = new SqlParameter("@ErrorType", SqlDbType.Int) { Direction = ParameterDirection.Output };
        command.Parameters.Add(outputMsg);
        command.Parameters.Add(outputErrorType);
        ...
        if (reader.Read())
        {
            insuranceDTO.ID = reader.IsDBNull(reader.GetOrdinal("ID")) ? 0 : reader.GetInt32(reader.GetOrdinal("ID"));
            insuranceDTO.CompanyName = reader.IsDBNull(reader.GetOrdinal("CompanyName")) ? null : reader.GetString(reader.GetOrdinal("CompanyName"));
            insuranceDTO.PolicyNumber = reader.IsDBNull(reader.GetOrdinal("PolicyNumber")) ? null : reader.GetString(reader.GetOrdinal("PolicyNumber"));
            insuranceDTO.CoveragePercentage = reader.IsDBNull(reader.GetOrdinal("CoveragePercentage")) ? 0 : reader.GetInt32(reader.GetOrdinal("CoveragePercentage"));
            insuranceDTO.DefaultDurationMonths = reader.IsDBNull(reader.GetOrdinal("DefaultDurationMonths")) ? 0 : reader.GetInt32(reader.GetOrdinal("DefaultDurationMonths"));
            insuranceDTO.Notes = reader.IsDBNull(reader.GetOrdinal("Notes")) ? null : reader.GetString(reader.GetOrdinal("Notes"));
            insuranceDTO.Price = reader.IsDBNull(reader.GetOrdinal("Price")) ? 0 : reader.GetDecimal(reader.GetOrdinal("Price"));
        }
        reader.Close();
        result.Data = insuranceDTO;
        result.Message = outputMsg.Value?.ToString();
        result.ErrorType = ErrorTypeMapper.GetErrorType(Convert.ToInt32(outputErrorType.Value));
    }
    catch (Exception ex)
...
public static async Task<ApiResult<List<InsuranceViewDTO>>> SearchInsurance(
string? CompanyName,
    ...
    CancellationToken cancellationToken = default)
{
    ...
    {
        await using var connection = new SqlConnection(connectionString._connectionString);
        await using var command = new SqlCommand("SP_SearchInsurance", connection) { CommandType = CommandType.StoredProcedure };
        command.Parameters.Add("@CompanyName", SqlDbType.NVarChar).Value = CompanyName ;
        command.Parameters.Add("@CoveragePercentage", SqlDbType.Int).Value = CoveragePercentage ;
        command.Parameters.Add("@DefaultDurationMonths", SqlDbType.Int).Value = DefaultDurationMonths ;
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var reader = await command.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false);
        while (reader.Read())
        {
