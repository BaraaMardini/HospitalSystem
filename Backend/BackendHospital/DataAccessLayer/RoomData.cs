...
RoomDTO roomDTO = new  RoomDTO();
    ...
    try
    {
        ...
        if (reader.Read())
        {
            roomDTO.ID = reader.IsDBNull(reader.GetOrdinal("ID")) ? 0 : reader.GetInt32(reader.GetOrdinal("ID"));
            roomDTO.RoomNumber = reader.IsDBNull(reader.GetOrdinal("RoomNumber")) ? null : reader.GetString(reader.GetOrdinal("RoomNumber"));
            roomDTO.Notes = reader.IsDBNull(reader.GetOrdinal("Notes")) ? null : reader.GetString(reader.GetOrdinal("Notes"));
            roomDTO.RoomTypeID = reader.IsDBNull(reader.GetOrdinal("RoomTypeID")) ? 0 : reader.GetInt32(reader.GetOrdinal("RoomTypeID"));
            roomDTO.IsActive = reader.IsDBNull(reader.GetOrdinal("IsActive")) ? false : reader.GetBoolean(reader.GetOrdinal("IsActive"));
        }
        reader.Close();
        result.Data = roomDTO;
        result.Message = outputMsg.Value?.ToString();
    ...
    CancellationToken cancellationToken = default)
{
    ...
    {
        await using var connection = new SqlConnection(connectionString._connectionString);
        await using var command = new SqlCommand("SP_SearchRoom", connection) { CommandType = CommandType.StoredProcedure };
