    ...
    CancellationToken cancellationToken = default)
{
    ...
    {
        await using var connection = new SqlConnection(connectionString._connectionString);
        await using var command = new SqlCommand("_SP_AddRole", connection) { CommandType = CommandType.StoredProcedure };
    ...
    {
        result.Data = null;
        result.Message = "Database error occurred while adding the role.";
        result.ErrorType = ErrorType.DatabaseError;
    }
    return result;
}

public static async Task<ApiResult<RoleDTO>> UpdateRoleByIDAsync(
    RoleUpdateDTO role,
    CancellationToken cancellationToken = default)
{
RoleDTO roleDTO = new  RoleDTO();
    ...
    try
    {
        ...
        if (reader.Read())
        {
            roleDTO.ID = reader.IsDBNull(reader.GetOrdinal("ID")) ? 0 : reader.GetInt32(reader.GetOrdinal("ID"));
            roleDTO.RoleName = reader.IsDBNull(reader.GetOrdinal("RoleName")) ? null : reader.GetString(reader.GetOrdinal("RoleName"));
            roleDTO.description = reader.IsDBNull(reader.GetOrdinal("description")) ? null : reader.GetString(reader.GetOrdinal("description"));
        }
        reader.Close();
        result.Data = roleDTO;
        result.Message = outputMsg.Value?.ToString();
